## ---------------------------
##
## Script name:   LMM_LumbarAccelerationIMU
##
## Purpose:       Linear mixed model to evaluate the association between propulsion force impulse and velocity increment derived
##                from anterior acceleration measured by a lumbar-mounted IMU in healthy and post-stroke individuals.
##
...
# Load dataset
filename = "Rdataset_LumbarAccelerationIMU.xlsx"
...
Data_selected <- Data %>%
  select(subject_name, trial, Group, leg, IMU_APAccelerationLumbar_VelocityIncrement_Value, PropulsionImpulse_Value) %>%
  rename(
    SubjectID = subject_name,
    Trial = trial,
    Group = Group,
    Leg = leg,
    VelocityIncrement = IMU_APAccelerationLumbar_VelocityIncrement_Value,
    PropulsionForce = PropulsionImpulse_Value
  ) %>%
  filter(!is.na(VelocityIncrement) & !is.na(PropulsionForce))
...
All_Data_selected <- Data %>%
  semi_join(Kept_combinations, 
            by = c("subject_name" = "SubjectID", 
                   ...
                   "Group" = "Group", 
                   "leg" = "Leg")) %>%
  filter(!is.na(IMU_APAccelerationLumbar_VelocityIncrement_Value) & 
...
Summary_parameters_per_subject <- All_Data_selected %>%
  ...
  summarise(
    ...
    Contralateral_StepLength_mean = mean(Contralateral_StepLength_Value, na.rm = TRUE),
    Contralateral_StepLength_sd   = sd(Contralateral_StepLength_Value, na.rm = TRUE),
    
    StrideLength_mean = mean(StrideLength_Value, na.rm = TRUE),
    StrideLength_sd   = sd(StrideLength_Value, na.rm = TRUE),
    
    Contralateral_StrideLength_mean = mean(Contralateral_StrideLength_Value, na.rm = TRUE),
    Contralateral_StrideLength_sd   = sd(Contralateral_StrideLength_Value, na.rm = TRUE),
    ...
    OMCS_APDecelerationSacrum_VelocityIncrement_mean = mean(OMCS_APDecelerationSacrum_VelocityIncrement_Value, na.rm = TRUE),
    OMCS_APDecelerationSacrum_VelocityIncrement_sd   = sd(OMCS_APDecelerationSacrum_VelocityIncrement_Value, na.rm = TRUE),
    
    OMCS_APDecelerationSacrum_Peak_mean = mean(OMCS_APDecelerationSacrum_Peak_Value, na.rm = TRUE),
    OMCS_APDecelerationSacrum_Peak_sd   = sd(OMCS_APDecelerationSacrum_Peak_Value, na.rm = TRUE),
    
    OMCS_APAccelerationSacrum_VelocityIncrement_mean = mean(OMCS_APAccelerationSacrum_VelocityIncrement_Value, na.rm = TRUE),
    OMCS_APAccelerationSacrum_VelocityIncrement_sd   = sd(OMCS_APAccelerationSacrum_VelocityIncrement_Value, na.rm = TRUE),
    
    OMCS_APAccelerationSacrum_Peak_mean = mean(OMCS_APAccelerationSacrum_Peak_Value, na.rm = TRUE),
    OMCS_APAccelerationSacrum_Peak_sd   = sd(OMCS_APAccelerationSacrum_Peak_Value, na.rm = TRUE),
    
    IMU_APDecelerationLumbar_VelocityIncrement_mean = mean(IMU_APDecelerationLumbar_VelocityIncrement_Value, na.rm = TRUE),
    IMU_APDecelerationLumbar_VelocityIncrement_sd   = sd(IMU_APDecelerationLumbar_VelocityIncrement_Value, na.rm = TRUE),
    
    IMU_APDecelerationLumbar_Peak_mean = mean(IMU_APDecelerationLumbar_Peak_Value, na.rm = TRUE),
    IMU_APDecelerationLumbar_Peak_sd   = sd(IMU_APDecelerationLumbar_Peak_Value, na.rm = TRUE),
    
    IMU_APAccelerationLumbar_VelocityIncrement_mean = mean(IMU_APAccelerationLumbar_VelocityIncrement_Value, na.rm = TRUE),
    IMU_APAccelerationLumbar_VelocityIncrement_sd   = sd(IMU_APAccelerationLumbar_VelocityIncrement_Value, na.rm = TRUE),
    
    IMU_APAccelerationLumbar_Peak_mean = mean(IMU_APAccelerationLumbar_Peak_Value, na.rm = TRUE),
    IMU_APAccelerationLumbar_Peak_sd   = sd(IMU_APAccelerationLumbar_Peak_Value, na.rm = TRUE),
...
for (w in step_conditions) {
  ...
  if (w == 1) {
    temp <- Data_selected %>%
      ...
      select(SubjectID, Trial, Group, Leg, VelocityIncrement, PropulsionForce, StepCondition, StrideNr_block)
    ...
    temp <- Data_selected %>%
      ...
      filter(n() == w) %>%
      summarise(
        VelocityIncrement = median(VelocityIncrement, na.rm = TRUE),
...
for (step in step_conditions) {
  ...
  for (group in names(groups)) {
    ...
    model <- lmer(PropulsionForce ~ VelocityIncrement + (1 + VelocityIncrement | SubjectID), data = data_group)
    ...
    sd_slope <- re_stddevs$sdcor[re_stddevs$var1 == "VelocityIncrement"]
    sd_slope <- sd_slope[1]
    ...
    fixed_slope <- fixef(model)["VelocityIncrement"]
    data_group <- data_group %>%
      mutate(
        FixedEffect_Predicted = fixed_intercept + fixed_slope * VelocityIncrement,
    ...
    Group_level_results[[paste0(step, "_", group)]] <- tibble(
      ...
      Slope_Est = fixed_effects["VelocityIncrement", "Estimate"],
      Slope_CI_low = fixed_effects["VelocityIncrement", "2.5%"],
      Slope_CI_high = fixed_effects["VelocityIncrement", "97.5%"],
      Slope_t = fixed_effects["VelocityIncrement", "t"],
      Slope_df = fixed_effects["VelocityIncrement", "df"],
      Slope_p = fixed_effects["VelocityIncrement", "p"],
    ...
    subject_specific_slopes <- coef(model)$SubjectID %>%
      ...
      select(SubjectID, LMM_Intercept = `(Intercept)`, LMM_Slope = VelocityIncrement)
...
ParticipantCharacteristics_summary_table <- ParticipantCharacteristics %>%
  ...
  summarise(
    ...
    `Comfortable walking speed (mean ± SD, m/s)` = if ("Comfortable walking speed [m/s]" %in% names(.)) {
      paste0(
        round(mean(`Comfortable walking speed [m/s]`, na.rm = TRUE), 2), " ± ",
        round(sd(`Comfortable walking speed [m/s]`, na.rm = TRUE), 2)
      )
    } else {"-"}
  )
...
make_plot <- function(group_name, step_condition_label) {
  ...
  x_range <- range(filtered_data$VelocityIncrement, na.rm = TRUE)
  x_vals <- seq(x_range[1], x_range[2], length.out = 100)
  line_data <- data.frame(
    VelocityIncrement = x_vals,
    PropulsionForce = intercept + slope * x_vals
  )
  filtered_data <- filtered_data %>%
    mutate(
      group_pred = intercept + slope * VelocityIncrement,
  ...
  individual_models <- filtered_data %>%
    group_by(SubjectID) %>%
    do(model = lm(PropulsionForce ~ VelocityIncrement, data = .))
  ...
  # Create plot
  ggplot(filtered_data, aes(x = VelocityIncrement, y = PropulsionForce, color = SubjectID)) +
  # Data points
  ...
  # Group-level R2 + RMSE
    geom_line(data = line_data, aes(x = VelocityIncrement, y = PropulsionForce), color = "black", size = 1.5, inherit.aes = FALSE) +
  # R2 labels
